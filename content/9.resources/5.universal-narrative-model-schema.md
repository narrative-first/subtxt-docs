...
### Canonical Standards (Base Set)  
The following values are **required** for compliance with the Universal Narrative Model. 

#### **Standard Appreciations (Base Set)**
...
**Validation Rule:** Any `appreciation`, `method`, `dynamic`, or `vector` **must match** one of the above unless explicitly mapped via the **custom mapping system**.

---

### Canonical Standards: Storybeats
...
"storybeats": {
  ...
  "items": {
    "type": "object",
    "properties": {
      "id": { "type": "string", "pattern": "^storybeat_[a-f0-9-]{36}$" },
      "scope": { "type": "string", "enum": ["transit", "progression", "event"] },
      "number": {
        "oneOf": [
          { "type": "integer", "minimum": 1, "maximum": 4, "description": "For Transits (1-4)" },
          { "type": "integer", "minimum": 1, "maximum": 16, "description": "For Progressions (1-16)" },
          { "type": "integer", "minimum": 1, "maximum": 64, "description": "For Events (1-64)" }
        ]
      },
      ...
      "perspective_id": { "type": ["string", "null"], "pattern": "^perspective_[a-f0-9-]{36}$" }
    },
    "required": ["id", "scope", "number", "appreciation", "method", "tones", "perspective_id"]
  }
}
```
---
...
If a user wants to use **alternative terminology**, they must define their custom term in **`custom_appreciation`**, **`custom_method`**, or **`custom_vector`** while still mapping it to an official term. If custom, they still must include the standard canonical item for consistent interoperability.
...
```json
{
  ...
  "story": {
    ...
    "narratives": [
      {
        "id": "narrative_abc123-def456-ghi789",
        "perspectives": [
          {
            "id": "perspective_ab12cd34-ef56-7890-gh12-ijkl34567890",
            "name": "Main Character",
            "description": "The perspective from which the audience primarily experiences the story.",
            "throughline": "Main Character"
          }
        ],
        "players": [
          {
            "id": "player_98abcd76-5432-1fef-9876-4567cdef2345",
            "name": "Dr. Michael Hayes",
            "role": "Main Character",
            "summary": "A psychologist haunted by his past struggles to guide a patient through similar trauma.",
            "storytelling": "A reserved but brilliant therapist, Michael projects confidence while hiding his own fears.",
            "perspective_id": "perspective_ab12cd34-ef56-7890-gh12-ijkl34567890",
            "motivations": [
              {
                "method": "Avoidance",
                "illustration": "Michael avoids discussing his past by focusing on his patient.",
                "storytelling": "In sessions, Michael deflects personal questions and steers the conversation elsewhere."
              }
            ]
          }
        ],
        "dynamics": [
          {
            "id": "dynamic_abcdef12-3456-7890-ghij-klmnopqrstuv",
            "appreciation": "Story Outcome",
            "vector": "Success",
            "summary": "The story resolves with Michael embracing his past and healing.",
            "storytelling": "Michael opens up to Sarah, allowing her progress to reflect his own growth.",
            "perspective_id": "perspective_ab12cd34-ef56-7890-gh12-ijkl34567890"
          }
        ]
      }
